...
const initializeSocket = (server) => {
    ...
    io.on("connection", async (socket) => {
        ...
        socket.on("create room", async (values) => {
            ...
            const room = new Room(values.roomName, values.roomNumCards, player.player_id)
            ...
            const gamePlayers = new GamePlayers(parseInt(gameCreated.game_id), parseInt(player.player_id))
        ...
        socket.on("joinRoom", async (roomId) => {
            try {
                ...
                const player = await Player.findPlayerById(socket.id);
                if (!player) {
                    throw new Error("Le joueur n'existe pas.");
                }

                // Mettre à jour les données de la salle pour inclure le joueur
                await GamePlayers.addPlayerToRoom(roomId, player.player_id);
